...
REPO_REMI="http://rpms.remirepo.net/enterprise/remi-release-${CENTOS_VERSION}.rpm"
...
REPO_WEBSCOOT_TMP="https://raw.githubusercontent.com/amitsdalal/lemp/master/"
NGINX_VERSION=$(curl -s http://nginx.org/en/download.html | grep -oP '(?<=gz">nginx-).*?(?=</a>)' | head -1)
NGINX_BASE="https://raw.githubusercontent.com/hyperxapps/webscoot-nginx/master/"
...
if [[ ${EUID} -ne 0 ]]; then
  ...
  GREENTXT "PASS: ROOT!"
  GREENTXT "     INSTALLING DNF for better package handling   "
  start_progress &
  pid="$!"
  yum install -y dnf >/dev/null 2>&1
  dnf  -y remove mariadb-libs  --noautoremove >/dev/null 2>&1  
  stop_progress "$pid"
  GREENTXT "INSTALLED: DNF!"
fi
...
if ! grep -q "webstack_is_clean" /opt/webscoot/cfg/.webstack >/dev/null 2>&1 ; then
installed_packages="$(rpm -qa --qf '%{name} ' 'mysqld?|firewalld|Percona*|maria*|php-?|nginx*|*ftp*|varnish*|certbot*|redis*|webmin')"
  ...
  echo "webstack_is_clean" > /opt/webscoot/cfg/.webstack
  fi
fi

GREENTXT "PATH: ${PATH}"
echo
if ! grep -q "yes" /opt/webscoot/cfg/.systest >/dev/null 2>&1 ; then
echo
...
echo
mkdir -p /opt/webscoot/cfg/ && echo "yes" > /opt/webscoot/cfg/.systest
echo
...
echo
# ssh test
if ! grep -q "yes" /opt/webscoot/cfg/.sshport >/dev/null 2>&1 ; then
...
if [ "${new_ssh_test}" == "y" ];then
	...
	YELLOWTXT "---> restrict SFTP users to their home directory"
        echo "yes" > /opt/webscoot/cfg/.sshport
	echo "SSH ${NEW_SSH_PORT}" >> /opt/webscoot/cfg/.sshport
	echo "SFTP ${SFTP_PORT}" >> /opt/webscoot/cfg/.sshport
	echo
	echo
	pause "---> Press [Enter] key to proceed"
        else
...
echo
if ! grep -q "yes" /opt/webscoot/cfg/.terms >/dev/null 2>&1 ; then
  YELLOWTXT "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
  echo
  ...
  if [ "${terms_agree}" == "y" ];then
    echo "yes" > /opt/webscoot/cfg/.terms
          else
...
printf "\033c"
    echo
      echo
        echo -e "${DGREYBG}${BOLD}  MAGENTO SERVER CONFIGURATION v.${WEBX_VER}  ${RESET}"
        BLUETXT ":::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::"
        echo
...
if ! grep -q "yes" /opt/webscoot/cfg/.sysupdate >/dev/null 2>&1 ; then
...
echo
curl -o /etc/motd -s ${REPO_WEBSCOOT_TMP}motd
sed -i "s/MAGE_VERSION_FULL/${MAGE_VERSION_FULL}/" /etc/motd
sed -i "s/WEBX_VER/${WEBX_VER}/" /etc/motd
echo "yes" > /opt/webscoot/cfg/.sysupdate
echo
fi
echo
echo
...
chmod 600 /root/.my.cnf /root/.mytop
              ...
              dnf -y install mytop >/dev/null 2>&1
...
if [ "${repo_remi_install}" == "y" ];then
      ...
      if [ "$?" = 0 ]
	    ...
	    echo
            ...
            long_progress &
            pid="$!"
            yum install -y php composer ${PHP_PACKAGES[@]/#/php-} ${PHP_PECL_PACKAGES[@]/#/php-} >/dev/null 2>&1
            stop_progress "$pid"
            rpm  --quiet -q php
       if [ "$?" = 0 ]
         then
         ...
         echo
           echo
            ...
            start_progress &
            pid="$!"
            yum -y --enablerepo=remi install redis >/dev/null 2>&1
            stop_progress "$pid"
            rpm  --quiet -q redis
       if [ "$?" = 0 ]
         then
...
echo
      ...
      if [ "$?" = 0 ]
        then
          echo
	          wget -qO /etc/systemd/system/varnish.service ${REPO_WEBSCOOT_TMP}varnish.service
            wget -qO /etc/varnish/varnish.params ${REPO_WEBSCOOT_TMP}varnish.params
...
GREENTXT "DOWNLOADING n98-MAGERUN2"
     curl -s -o /usr/local/bin/magento2-cli https://files.magerun.net/n98-magerun2.phar
echo
...
cp composer.json /opt/webscoot/composer.json.saved
cp composer.lock /opt/webscoot/composer.lock.saved
cp app/etc/env.php /opt/webscoot/env.php.saved
chmod -R 600 /opt/webscoot
echo
echo
GREENTXT "FIXING PERMISSIONS"
chmod -R 600 /opt/webscoot/cfg
...
[installed db dump]: /opt/webscoot/${MAGE_DB_NAME}.sql.gz
[composer.json copy]: /opt/webscoot/composer.json.saved
[env.php copy]: /opt/webscoot/env.php.saved
[env.php default copy]: /opt/webscoot/env.php.default
...
if [ "${frwlltst}" == "csf" ];then
		   ...
		   curl -o /usr/local/csf/bin/regex.custom.pm ${REPO_WEBSCOOT_TMP}regex.custom.pm
